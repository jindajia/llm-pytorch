...
from .utils import Split, log_single_rank
...
class GPTDataset(MegatronDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        attention_mask = torch.ones(tokens.numel(), dtype=torch.int, device=tokens.device)
    ...
    def _build_document_sample_shuffle_indices(
        ...
        log_single_rank(
            logger,
            logging.INFO,
            f"number of tokens per epoch {num_tokens_per_epoch}",
        )
        ...
        if not cache_hit:
            ...
            document_index = _build_document_index(
                self.indexed_indices, num_epochs, separate_final_epoch
            )
...
def _build_document_index(
    ...
    doc_idx_first = _build_document_index(documents, num_epochs - 1, False)
    doc_idx_last = _build_document_index(documents, 1, False)
